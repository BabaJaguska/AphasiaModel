...
class LanguageProcessingModel:
    ...
    def _decoder(self, embeddings):        
        ...
        batch_size = 1
        decoder_input_ids = torch.ones((batch_size, 1),
                                       dtype=torch.long,
                                       device=self.model.device) * decoder_start_token_id
        ...
        for _ in range(self.max_tokens):
            ...
            decoder_input_ids = torch.cat([decoder_input_ids, predicted_token_id], dim=-1)
        generated_sequence = torch.cat(generated_sequence, dim=-1)
        decoded_output = self.tokenizer.batch_decode(generated_sequence,
    ...
    def hinder_utterance(self, tokens):
        ...
        content_tokens = [token for token in tokens if token.lower() not in stop_words]
        brocas_utterances = []
        i = 0
        ...
        messed_up_tokens = brocas_aphasia_tokens[:-1]   # Remove the last "..." from the tokens
    ...
    def articulation(self, sentence, aphasia=None):
        tokens = self.tokenizer.tokenize(sentence, return_tensors="pt", add_special_tokens=False)
        if aphasia == 'Broca':
            tokens, sentence = self.hinder_utterance(tokens)
        phonetic_representation = [doublemetaphone(token) for token in tokens]
    ...
    def process(self, in_sentence, aphasia=None, verbose=True):
        ...
        phonetic_representation, pronounced_sentence = language_model.articulation(gen_sentence, aphasia)
        pathology = 'HEALTHY'
        if aphasia == "Wernicke": pathology = "WERNICKE'S APHASIA"
        elif aphasia == "Broca": pathology = "BROCA'S APHASIA"
        if verbose: print(f'{pathology:20}: {pronounced_sentence}\n{phonetic_representation}')
